...
class KDTree
{
  ...
  public:
     ...
     *  Point::operator<().
     */
    //my dumping ground for all the functions i made (:
    bool shouldReplace(const Point<Dim>& target, const Point<Dim>& currentBest,
                       const Point<Dim>& potential) const;
    int partition( vector<Point<Dim>>& newPoints,int left, int  right, unsigned pivotIndex,unsigned dim);
    Point<Dim> quickselect( vector<Point<Dim>>& newPoints,int  left, int  right, unsigned k, unsigned dim);
    void copy(KDTreeNode * subroot, KDTreeNode * othersubroot);
    typename KDTree<Dim>::KDTreeNode* recursiveConstructor(vector<Point<Dim>>& newPoints,int  left,int  right,int depth);
    void clear(KDTreeNode * first);
    void findNN(KDTreeNode* curRoot, Point<Dim> &currentBest,
                         Point<Dim> &query, int dimension) const;

    Point<Dim> findNearestHelper2(KDTreeNode * current, const Point<Dim>& query, Point<Dim>& best, size_t depth) const;
  
  

    double length(KDTreeNode * current, const Point<Dim>& query, int depth, Point<Dim>& best) const;
    void assignHalves(KDTreeNode * current, const Point<Dim>& query, int depth, KDTreeNode* & first, KDTreeNode* & second);
    /**
